...
#        return sum_grad_norm / self.args.n_pc
    ...
    def forward(self, mol_graph1, mol_graph2, debug=False):
        ...
        atom_h1, _ = self.gcn(mol_graph1)
        atom_h2, _ = self.gcn(mol_graph2)

        # scope delienates the boundaries of the molecules in atom_h
        scope1 = mol_graph1.scope
        scope2 = mol_graph2.scope
...
#            self.pc_list = self.get_graph_pc_embeddings(MolGraph(self.args.pc_data[0]))
        ...
        for mol_idx, (st, le) in enumerate(scope1):
            # Fetch the atoms for the corresponding molecular graph
            atom_X1 = atom_h1.narrow(0, st, le)
            # Construct uniform prob dist over the atoms for this molecule
            atom_H1 = np.ones([le]) / le
                
            cur_dist_list = []
            for mol_idx2, (st2, le2) in enumerate(scope2):
                # Fetch the atoms for the corresponding molecular graph
                atom_X2 = atom_h2.narrow(0, st2, le2)
                # Construct uniform prob dist over the atoms for this molecule
                atom_H2 = np.ones([le2]) / le2    
...
#        print(' ')
                if mol_idx == mol_idx2:
                    ...
                    if self.args.distance_metric == 'wasserstein':
                        ot_dist, nce_reg, ot_mat, cost_mat = compute_ot(
                            X_1=atom_X1, X_2=atom_X2, H_1=atom_H1, H_2=atom_H2,
            ...
            #                ot_dist = ot_dist * pc_X.size()[0] * atom_X.size()[0] / 100
                        ...
                        if self.args.agg_func == 'sum':
                            mol_X1 = torch.sum(atom_X1, dim=0)
                            mol_X2 = torch.sum(atom_X2, dim=0)
                        elif self.args.agg_func == 'mean':
                            mol_X1 = torch.mean(atom_X1, dim=0)
                            mol_X2 = torch.sum(atom_X2, dim=0)
                        else:
                            assert False
                        l2_dist = torch.sum((mol_X1 - mol_X2) ** 2)
                        ...
                        if self.args.agg_func == 'sum':
                            ...
                            mol_X1 = torch.mean(atom_X1, dim=0)
                            mol_X2 = torch.mean(atom_X2, dim=0)
                        else:
                            assert False
                        dot_dist = torch.sum(mol_X1 * mol_X2)
...
#            assert(False)
        ...
        output_dict.update(
            {'preds': preds, 'atom_h': (atom_h1, atom_h2), 'nce_reg': nce_reg})

        if debug:
            pdb.set_trace()
        print('proto_net is ok')
        return output_dict

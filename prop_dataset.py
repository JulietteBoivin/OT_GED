...
#                        
        df = pd.read_csv('%s/raw.csv' % data_dir, header = 0, index_col = 0)
        for ind1, ind2 in data_indices:
            smile1, smile2 = ind1, ind2
            label = df.loc[ind1, ind2]
            label = float(label)
            self.data.append((smile1, smile2, label))
        
        print('csv and json loaded')

        self.smile1, self.smile2, self.label = zip(*self.data)
...
def get_loader(data_dir, data_type, batch_size, split=0, parse_func=None,
    ...
    def combine_data(data):
        if n_labels == 1:
            batch_smiles1, batch_smiles2, batch_labels = zip(*data)
            return batch_smiles1, batch_smiles2, batch_labels
        else:
            batch_smiles1, batch_smiles2, batch_labels, mask = zip(*data)
            return batch_smiles1, batch_smiles2, batch_labels, mask
